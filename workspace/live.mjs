...
export function renderDraftPreviews(drafts) {
  ...
  return `<section class="live-construction" data-artifact-count="${drafts.length}"><header class="live-construction-head"><div><span class="live-kicker">LIVE DESIGN CANVAS</span><h3>${escape(current.name||'Design direction')}</h3><p>${escape(current.caption||'A real design artifact is being checked before selection.')}</p></div><span class="live-artifact-count">${String(drafts.length).padStart(2,'0')} / 03 ready</span></header><div class="live-canvas-shell"><div class="live-canvas-chrome"><span><i></i><i></i><i></i></span><b>Latest verified artifact</b><em>Desktop</em></div><div class="live-canvas-media">${image?`<img src="${escape(image.url)}" alt="${escape(current.name||'Design direction')} work in progress" loading="eager" referrerpolicy="no-referrer">`:''}<span class="live-canvas-scan" aria-hidden="true"></span></div></div><div class="live-artifact-rail" aria-label="Available design artifacts">${drafts.map((draft,index)=>`<article class="live-artifact ${index===drafts.length-1?'is-current':''}">${draft.featured_previews[0]?`<img src="${escape(draft.featured_previews[0].url)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:''}<div><span>${String(index+1).padStart(2,'0')}</span><b>${escape(draft.name||'Design direction')}</b><small>${index===drafts.length-1?'On canvas':'Checked'}</small></div></article>`).join('')}</div></section>`;
}
...
export function mountLiveProject(container,{token,endpoint,onChange=()=>{}}) {
  ...
  function render(data){
    ...
    const waitingTitle=data.payment_required||data.stage==='payment'?'Authorize your project.':data.stage==='research'?'We’re learning what makes the business credible.':data.stage==='directions'?'The first design artifact will appear here.':data.stage==='building'?'The first working page will appear here.':'This is your project workspace.';
    const waitingCopy=data.payment_required||data.stage==='payment'?'Your brief is saved. No research, design generation, or charge has run yet. Authorize the project when you are ready to begin.':data.stage==='research'?'Nothing visual has been produced yet. The canvas opens when the first checked design artifact exists.':data.stage==='directions'?'Each real preview appears independently after it passes its checks. You choose the direction for the full build.':data.stage==='building'?'The canvas opens with the first published working preview, then keeps the browsing context as new builds arrive.':'Research, design choices and previews will appear here as they become available.';
